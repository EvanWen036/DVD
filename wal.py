...
class WriteAheadLog:
    def __init__(self):
        ...
        self._replica_lsn: Dict[str, int] = {}  # replica_id -> last applied lsn
    ...
    def next_from(self, last_lsn: int) -> LogRecord:
        """Block until there is a record with lsn > last_lsn, then return it."""
        want = last_lsn + 1
        with self._lock:
    ...
    def wait_replicated(self, lsn: int, min_replicas: int) -> None:
        """Block until at least `min_replicas` replicas have applied >= lsn."""
        with self._lock:
            while True:
    ...
    def latest_lsn(self) -> int:
        """Return the largest lsn that exists, or -1 if empty."""
        with self._lock:
            return len(self._records) - 1

    def replica_lsn(self, replica_id: str) -> int:
        """Return last lsn applied by this replica, or -1 if none."""
        with self._lock:
